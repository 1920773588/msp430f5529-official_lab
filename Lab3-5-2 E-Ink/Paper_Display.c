...
void INIT_SSD1673()
{
    MyRESET();
        ...
        SPI4W_WRITECOM(0x21);       // Option for Display Update
        SPI4W_WRITEDATA(0x83);    // A[7]=1(Enable bypass)  A[4]=0全黑(value will be used as for bypass)
        DIS_IMG(3);         // 全黑到全白清屏，这样可防止开机出现花屏的问题
...
void SPI4W_WRITECOM(unsigned char INIT_COM)//传输命令
{
...
void SPI4W_WRITEDATA(unsigned char INIT_DATA)//传输数据
{
	unsigned char TEMPCOM;
	unsigned char scnt;
	TEMPCOM=INIT_DATA;
			 ...
			 )
{
	...
	while(*str)
	{
	   ...
	   //因为在ASCii中，！是0x21，对应表中的第2个字符，（*str）去的是字符，运算时是自负的ASCii编码
